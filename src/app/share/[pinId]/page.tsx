...
export default function SharePinClient() {
  ...
  const [userDetails, setuserDetails] = useState<{ displayName: string; photoURL: string }>({ displayName: '', photoURL: '' });
...
function NoImagesDisplay({ pin, setImages }: NoImagesDisplayProps) {
  ...
  const fetchGooglePlaceImages = async () => {
    ...
    try {
        ...
        (place, status) => {
          if (status === google.maps.places.PlacesServiceStatus.OK && place) {
            const photos = place.photos 
              ? place.photos.map((photo) => photo.getUrl({ maxWidth: 600, maxHeight: 600 })) 
              : [];
      ...
      setError(`Error generating images: ${err instanceof Error ? err.message : String(err)}`);
      setIsLoading(false);
    }
  };
